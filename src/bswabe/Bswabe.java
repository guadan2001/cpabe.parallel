...
//		DefaultCurveParameters params = (DefaultCurveParameters) cg.generate();
	...
	public static BswabePrv keygen(BswabePub pub, BswabeMsk msk, String[] attrs)
		...
		System.err.println("G1 Length:" + pairing.getG1().getLengthInBytes()*8 + " bits");
		System.err.println("G2 Length:" + pairing.getG2().getLengthInBytes()*8 + " bits");
...
//		decFlatten(t, cph.p, prv, pub);
	...
	private static void decFlattenOptimized(Element r, BswabePolicy p, BswabePrv prv,
			BswabePub pub) throws Exception {
		Element one;
		one = pub.p.getZr().newElement();
		one.setToOne();
		r.setToOne();
		...
		protected void compute() {
			...
			else
			{
				...
				for (int i = 0; i < p.satl.size(); i++) {
					...
					expnew = exp.duplicate();
					expnew.mul(t);
					new DecNodeFlattenTask(r, expnew, p.children[p.satl.get(i) - 1], prv, pub).fork();
				}
			}
